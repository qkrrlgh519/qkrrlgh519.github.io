...
- 찾기 연산이 빨라서 좋지만, 합차기 연산을 거의 쓰지 않는다면 모를까 쓰기가 힘들다. 그래서 찾기 연산에 시간이 좀 더 걸려도 합치기 연산을 빠르게 할 수 있는 방법이 필요하다.
...
- **찾기(find) 연산**
  ...
  - 이런 찾기 연산을 구현하기 위해서는 모든 자식 노드가 부모에 대한 포인터를 가지고 있어야 한다. 반면 부모에서 자식으로 내려갈 일은 없기 때문에 부모는 자식에 대한 포인터를 가지고 있을 필요가 없다. (그림에서 모든 화살표가 자식에서 부모로 연결되는 이유)
...
### 02. 해결 1 : 랭크에 의한 합치기 (union-by-rank) 최적화
